...
/*   Updated: 2026/01/22 00:33:45 by gabo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
	...
	// ===================== INTERN TESTS (ENGLISH) =====================

	// TEST: Intern creates and executes ShrubberyCreationForm
	{
		std::cout << "\n=== TEST: Intern creates and executes ShrubberyCreationForm ===" << std::endl;
		Intern intern;
		Bureaucrat boss("Boss", 1);
		AForm *form = intern.makeForm(std::string("shrubbery creation"), std::string("Garden"));
		if (form) {
			boss.signForm(*form);
	...
	// TEST: Intern creates and executes RobotomyRequestForm
	{
		std::cout << "\n=== TEST: Intern creates and executes RobotomyRequestForm ===" << std::endl;
		Intern intern;
		Bureaucrat boss("Boss", 1);
		AForm *form = intern.makeForm(std::string("robotomy request"), std::string("House"));
		if (form) {
			boss.signForm(*form);
			boss.executeForm(*form);
			delete form;
	...
	// TEST: Intern creates and executes PresidentialPardonForm
	{
		std::cout << "\n=== TEST: Intern creates and executes PresidentialPardonForm ===" << std::endl;
		Intern intern;
		Bureaucrat boss("Boss", 1);
		AForm *form = intern.makeForm(std::string("presidential pardon"), std::string("Alice"));
		if (form) {
			boss.signForm(*form);
			boss.executeForm(*form);
			delete form;
	...
	// TEST: Intern receives an unknown form type
	{
		std::cout << "\n=== TEST: Intern receives unknown form type ===" << std::endl;
		Intern intern;
		AForm *form = intern.makeForm(std::string("unknown form"), std::string("Target"));
		if (!form) {
			std::cout << "Correctly did not create an unknown form type." << std::endl;
		}
	}
